...
class MultiTransform:
    ...
    def __call__(self, img):
        return [transform(img) for transform in self.transforms]
...
class CustomCub2011(Dataset):
    ...
    def __init__(self, args, root, train=True, transform=None, target_transform=None, loader=default_loader, download=True):
        ...
        if args.n_intra != 1:
            self.transform = MultiTransform([transform] * args.n_intra)
            self.target_transform = MultiTransform([target_transform] * args.n_intra)   
...
def get_cub_datasets(args, train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0, download=False, data_root=None):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = CustomCub2011(args, root=data_root, transform=train_transform, train=True, download=download)
    ...
    whole_test_dataset = CustomCub2011(args, root=data_root, transform=test_transform, train=False)
...
if __name__ == '__main__':
    args = 0
    x = get_cub_datasets(args, None, None, split_train_val=False,

...
seed=1026
output_dir=exp/
run_name="cub_seed($seed)_zs3072_l1recon"
#run_name=test

# non-restricted version: CUDA_VISIBLE_DEVICES=$(nvidia-smi --query-gpu=index,memory.used --format=csv,noheader,nounits | sort -t ',' -k2 -n | head -n 1 | awk -F ',' '{print $1}')
